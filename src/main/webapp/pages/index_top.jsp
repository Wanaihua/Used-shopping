...
<%
    ...
    if(request.getAttribute("useid")!=null){
        id=Integer.parseInt(request.getAttribute("useid").toString());
    }else if(request.getParameter("useid")!=null){
        id=Integer.parseInt(request.getParameter("useid"));
    }
%>
<jsp:include page="${pageContext.request.contextPath}/pages/topbar_1.jsp">
<jsp:param name="useid" value="<%=id %>" />
</jsp:include>
<div id="headerWrap">
...
<%
    ...
    for (int i = 1; i <= pages; i++) { // 通过循环构建分页导航条
        if (i == currPage) { // 判断是否为当前页
            ...
            // 构建分页导航条
            sb.append("<a href='/pages/index_top.jsp?useid="+id+"&page=" + i + "'>" + i + "</a>");
        }
        sb.append("　"); // 构建分页导航条
    }
 ...
 %>
	...
	<table id="tab" width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#666666" class="show">
		...
		<tr align="center">
			...
			<td width="15%"><form action="/account/information" method="post" onsubmit="return check(this);">
			...
			<td width="15%"><form action="/account/Shopcar" target="_bank" method="post" onsubmit="return check(this);">
