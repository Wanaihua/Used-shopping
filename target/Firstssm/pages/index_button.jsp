...
%>
<jsp:include page="${pageContext.request.contextPath}/pages/topbar_3.jsp">
<jsp:param name="useid" value="<%=useid %>" />
</jsp:include>
<div id="headerWrap">
    <div id="homeWrap" class="wrapper">
        ...
        <div id="header">
            <div id="headerinside">
                <a href="${pageContext.request.contextPath}/pages/index_button.jsp?useid=<%= useid%>" target="_self" id="logo">
        ...
        <ul class="navcon" id="nav" style="margin-top:20px">
            <li id="homeNav" class="navBg fl">
            	<a class="navWit" href="${pageContext.request.contextPath}/pages/index_button.jsp?useid=<%= useid%>" target="_self">首页</a>
            </li>
        </ul>
    </div>
</div>
...
<%
    ...
    for (int i = 1; i <= pages; i++) { // 通过循环构建分页导航条
        if (i == currPage) { // 判断是否为当前页
            ...
            // 构建分页导航条
            sb.append("<a href='/pages/index_button.jsp?page="+i+"&useid=" + useid + "'>" + i + "</a>");
        }
        sb.append("　"); // 构建分页导航条
    }
 ...
 %>
	<div class="div1">
	<table id="tab" width="98%" border="0" align="center" cellpadding="0"
		...
		<%
			...
			if (list1 == null || list1.size() < 1) {
				out.print("<tr><td bgcolor='#FFFFFF' colspan='5'>没有发布商品！</td></tr>");
			} else {
				// 遍历图书集合中的数据
		...
		<tr align="center">
			...
			<td bgcolor="#FFFFFF"  width="30%"><%=goods.getDdescribe()%></td>
			<td  width="10%"><form action="account/information" method="post" onsubmit="return check(this);">
			<input type="hidden" value="<%=goods.getUseid()%>" name="useid">
			<input type="hidden" value="<%=useid%>" name="FromId">
			<input type="submit" value="卖家信息" class="hover">
			</form>
			</td>
			<td  width="10%"><form action="account/Shopcar" target="_bank" method="post" onsubmit="return check(this);">
			<input type="hidden" value="<%=useid%>" name="uuid">
			<input type="hidden" value="<%=goods.getId()%>" name="id">
			<input type="submit" value="加入购物车"  class="hover">
			</form>
			</td>
			<!--<td bgcolor="#FFFFFF"><form action="account/US" method="post">
        	<input type="hidden" name="id" value="<%=goods.getId()%>">
        	<input type="hidden" name="useid" value="<%=goods.getUseid()%>">
        	<input type="hidden" name="iid" value="<%=useid%>">
        	<input type="text" name="price" size="3">
        	<input type="submit" value="修　改" class="hover">
        	</form></td>-->
        	<td bgcolor="#FFFFFF"  width="10%"><form action="account/DS" method="post">
        	<input type="hidden" name="id" value="<%=goods.getId()%>">
        	<input type="hidden" name="useid" value="<%=goods.getUseid()%>">
        	<input type="hidden" name="iid" value="<%=useid%>">
